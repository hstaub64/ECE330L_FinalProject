...
>>>>>>> 11e06f2 (Updated draw board method and main gameplay loop)
    ...
    {
      // top seg
      if (m.horizontal[0][i] != null)
        board[i] |= (1 << 0);
      // middle seg
      if (m.horizontal[1][i] != null)
        board[i] |= (1 << 6);
      // bottom seg
      if (m.horizontal[2][i] != null)
        board[i] |= (1 << 3);
    }
    ...
    {
      // top left
      if (m.vertical[0][i] != null && (i % 2) == 0)
        board[i] |= (1 << 5);
      // bottom left
      if (m.vertical[1][i] != null && (i % 2) == 0)
        board[i] |= (1 << 4);
      // top right
      if (m.vertical[0][i] != null && (i != 0))
        board[i] |= (1 << 1);
      // bottom right
      if (m.vertical[1][i] != null && (i % 2) != 0)
        board[i] |= (1 << 2);
